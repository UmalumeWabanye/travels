...
export function StoryParallax() {
  ...
  return (
    <section ref={sectionRef} className="section-rhythm-wide relative overflow-hidden bg-[var(--surface)] px-6 text-[var(--foreground)] md:px-10">
      <div className="story-background absolute inset-0 bg-[var(--surface)]" />
      <div className="relative mx-auto max-w-6xl">
        <p className="font-[var(--font-nav)] text-xs uppercase tracking-[0.28em] text-zinc-500">Storytelling Scroll</p>
        <h2 className="mt-3 font-[var(--font-playfair)] text-4xl md:text-6xl">Every Destination, A Cinematic Chapter</h2>
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {chapters.map((chapter) => (
            <article key={chapter.title} className="chapter-item rounded-2xl border border-[var(--border-soft)] bg-white p-6">

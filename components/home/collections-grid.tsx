  ...
  return (
    <section className="section-rhythm-regular bg-(--surface) px-6 text-foreground md:px-10">
      <div className="mx-auto max-w-7xl">
        <p className="font-(--font-nav) text-xs uppercase tracking-[0.25em] text-zinc-500">Collections</p>
        <h2 className="mt-2 font-(--font-playfair) text-4xl md:text-5xl">Travel edits for every mood</h2>
          ...
          {travelCollections.slice(0, 9).map((collection, index) => (
            <article key={collection.id} className="vivid-card overflow-hidden rounded-[1.75rem]">
              ...
              <div className="p-5 bg-white">
                <p className="text-xl text-foreground">{collection.title}</p>

  ...
  return (
    <section className="section-rhythm-tight bg-(--surface) px-6 text-foreground md:px-10">
      <div className="mx-auto max-w-7xl">
        <Badge className="text-zinc-700">Design Language</Badge>
        <h2 className="mt-4 max-w-3xl font-(--font-playfair) text-4xl md:text-6xl">
          Built from modern product storytelling references
        </h2>
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {principles.map((principle) => (
            <article key={principle.title} className="rounded-2xl border border-(--border-soft) bg-white p-6">
              <h3 className="text-xl">{principle.title}</h3>
              <p className="mt-3 text-sm leading-relaxed text-zinc-600">{principle.text}</p>
            </article>
          ))}
        </div>
      </div>
